  ...
  render() {
    ...
    return <div>
      { podcasts.map((podcast) => (
        // Aquí ser+a necesario construir el link a mano
        <a href={`/${slug(podcast.channel.title)}.${podcast.channel.id}/${slug(podcast.title)}.${podcast.id}`}
          className='podcast' key={podcast.id}
          // El evento onClick lo que hace es tomar una función en donde le pasamos el evento y la información del podcast
